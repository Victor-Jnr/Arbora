...
def test_wifi_status_is_read_only_inspect():
    ...
    assert any(step.action == "inspect_battery" for step in diagnose.steps)
    assert len(diagnose.steps) > 1
    assert all(step.sensitivity == Sensitivity.READ for step in diagnose.steps)


def test_battery_status_is_read_only_inspect():
    runtime = _runtime()
    plan = runtime.planner.plan("battery status")
    assert [step.action for step in plan.steps] == ["inspect_battery"]
    assert plan.steps[0].sensitivity == Sensitivity.READ
    assert not plan.has_hard_confirmation_steps
    diagnose = runtime.planner.plan("diagnose battery")
    assert any(step.action == "inspect_battery" for step in diagnose.steps)
    assert len(diagnose.steps) > 1
...
def test_volume_is_read_only_inspect():
    runtime = _runtime()
    plan = runtime.planner.plan("volume")
    assert [step.action for step in plan.steps] == ["inspect_volume"]
    assert plan.steps[0].adapter == "desktop"
    assert plan.steps[0].sensitivity == Sensitivity.READ
    assert not plan.has_hard_confirmation_steps
    named = runtime.planner.plan("what's my volume")
    assert named.steps[0].action == "inspect_volume"
    muted = runtime.planner.plan("am i muted")
    assert muted.steps[0].action == "inspect_volume"
    change = runtime.planner.plan("set volume to 50")
    assert not any(step.action == "inspect_volume" for step in change.steps)
    mute_action = runtime.planner.plan("mute the speakers")
    assert not any(step.action == "inspect_volume" for step in mute_action.steps)
    diagnose = runtime.planner.plan("diagnose disk space")
    assert not any(step.action == "inspect_volume" for step in diagnose.steps)
...
def test_power_plan_is_read_only_inspect():
    runtime = _runtime()
    plan = runtime.planner.plan("power plan")
    assert [step.action for step in plan.steps] == ["inspect_power_plan"]
    assert plan.steps[0].adapter == "desktop"
    assert plan.steps[0].sensitivity == Sensitivity.READ
    assert not plan.has_hard_confirmation_steps
    named = runtime.planner.plan("what power plan am i on")
    assert named.steps[0].action == "inspect_power_plan"
    scheme = runtime.planner.plan("inspect power plan")
    assert scheme.steps[0].action == "inspect_power_plan"
    diagnose = runtime.planner.plan("diagnose disk space")
    assert not any(step.action == "inspect_power_plan" for step in diagnose.steps)
    mutate = runtime.planner.plan("set power plan to high performance")
    assert not any(step.action == "inspect_power_plan" for step in mutate.steps)
    battery = runtime.planner.plan("battery status")
    assert [step.action for step in battery.steps] == ["inspect_battery"]
...
def test_secure_boot_is_read_only_inspect():
    runtime = _runtime()
    plan = runtime.planner.plan("secure boot")
    assert [step.action for step in plan.steps] == ["inspect_secure_boot"]
    assert plan.steps[0].adapter == "desktop"
    assert plan.steps[0].sensitivity == Sensitivity.READ
    assert not plan.has_hard_confirmation_steps
    named = runtime.planner.plan("is secure boot on")
    assert named.steps[0].action == "inspect_secure_boot"
    enabled = runtime.planner.plan("is secure boot enabled")
    assert enabled.steps[0].action == "inspect_secure_boot"
    diagnose = runtime.planner.plan("diagnose disk space")
    assert not any(step.action == "inspect_secure_boot" for step in diagnose.steps)
    mutate = runtime.planner.plan("disable secure boot")
    assert not any(step.action == "inspect_secure_boot" for step in mutate.steps)
