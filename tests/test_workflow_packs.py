...
def test_inspect_idle_workflow_pack_matches():
    pack = match_workflow_pack("run inspect idle pack")
    assert pack is not None
    assert pack.id == "inspect-idle"
    plan = pack.to_plan("run inspect idle pack")
    assert plan is not None
...
def test_inspect_audio_device_workflow_pack_matches():
    ...
    assert pack is not None
    assert pack.id == "inspect-audio-device"
    plan = pack.to_plan("run inspect audio device pack")
    assert plan is not None
...
def test_inspect_installed_apps_workflow_pack_matches():
    pack = match_workflow_pack("run inspect installed apps pack")
    assert pack is not None
    assert pack.id == "inspect-installed-apps"
    plan = pack.to_plan("run inspect installed apps pack")
    assert plan is not None
...
def test_inspect_windows_version_workflow_pack_matches():
    pack = match_workflow_pack("run inspect windows version pack")
    assert pack is not None
    assert pack.id == "inspect-windows-version"
    plan = pack.to_plan("run inspect windows version pack")
    assert plan is not None
...
def test_inspect_pending_reboot_workflow_pack_matches():
    pack = match_workflow_pack("run inspect pending reboot pack")
    assert pack is not None
    assert pack.id == "inspect-pending-reboot"
...
def test_inspect_disk_space_workflow_pack_matches():
    pack = match_workflow_pack("run inspect disk space pack")
    assert pack is not None
    assert pack.id == "inspect-disk-space"
...
def test_inspect_power_plan_workflow_pack_matches():
    pack = match_workflow_pack("run inspect power plan pack")
    assert pack is not None
    assert pack.id == "inspect-power-plan"
    plan = pack.to_plan("run inspect power plan pack")
    assert plan is not None
    assert [step.action for step in plan.steps] == ["inspect_power_plan"]
    assert plan.steps[0].adapter == "desktop"
    assert plan.steps[0].sensitivity.value == "read"


def test_inspect_cpu_workflow_pack_matches():
    pack = match_workflow_pack("run inspect cpu pack")
    assert pack is not None
    assert pack.id == "inspect-cpu"
    plan = pack.to_plan("run inspect cpu pack")
    assert plan is not None
...
def test_inspect_secure_boot_workflow_pack_matches():
    pack = match_workflow_pack("run inspect secure boot pack")
    assert pack is not None
    assert pack.id == "inspect-secure-boot"
...
def test_inspect_bluetooth_workflow_pack_matches():
    pack = match_workflow_pack("run inspect bluetooth pack")
    assert pack is not None
    assert pack.id == "inspect-bluetooth"
    plan = pack.to_plan("run inspect bluetooth pack")
    assert plan is not None
...
def test_inspect_gpu_workflow_pack_matches():
    pack = match_workflow_pack("run inspect gpu pack")
    assert pack is not None
    assert pack.id == "inspect-gpu"
    plan = pack.to_plan("run inspect gpu pack")
    assert plan is not None
...
def test_inspect_airplane_workflow_pack_matches():
    pack = match_workflow_pack("run inspect airplane pack")
    assert pack is not None
    assert pack.id == "inspect-airplane"
